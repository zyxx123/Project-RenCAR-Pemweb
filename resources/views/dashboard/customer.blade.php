            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-sm font-semibold text-slate-500 uppercase tracking-wide mt-1">Total Bookings</p>
                    </div>
                </div>
                <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm relative overflow-hidden">
                    <div class="absolute top-0 right-0 p-4 opacity-10">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-sm font-semibold text-slate-500 uppercase tracking-wide mt-1">Completed Trips</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-slate-100">
                <div class="p-8 text-slate-900">
                    <div class="flex justify-between items-center mb-8 border-b border-slate-100 pb-4">
                        <h3 class="text-xl font-bold text-slate-800">Booking History</h3>
                        <a href="{{ route('home') }}" class="bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md hover:shadow-blue-500/30 hover:bg-blue-700 transition-all">Book Another Vehicle</a>
                    </div>
                    ...
                    @if($bookings->isEmpty())
                        <div class="text-center py-12">
                            ...
                            <p class="text-slate-500 mb-4 text-lg">You have not made any bookings yet.</p>
                            <a href="{{ route('home') }}" class="text-blue-600 hover:text-blue-700 font-semibold hover:underline">Explore our vehicles</a>
                        </div>
                    @else
                            ...
                            @foreach($bookings as $booking)
                                <div class="bg-white border border-slate-200 rounded-2xl p-6 relative shadow-sm hover:shadow-md transition-shadow group">
                                    ...
                                    <div class="grid grid-cols-2 gap-4 text-sm mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
                                        <div>
                                            <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Pickup</p>
                                            <p class="font-bold text-slate-800">{{ \Carbon\Carbon::parse($booking->start_date)->format('M d, Y') }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Return</p>
                                    ...
                                    <div class="border-t border-slate-100 pt-5 flex justify-between items-center">
                                        <div>
                                            <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Total Price</p>
                                            <p class="font-black text-lg text-slate-900">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</p>
                                        </div>
                                        
                                        @if($booking->status === 'pending')
                                            <a href="{{ route('booking.payment', $booking->id) }}" class="bg-amber-500 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-md hover:bg-amber-600 hover:shadow-amber-500/30 transition-all">Pay Now</a>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
